...
__global__ void softmax3_kernel(const float* x, float* y, int rows, int cols) {
    ...
    for (int c = tid; c < cols; c += block_size) {
        int idx = row * cols + c;
        float x_val = x[idx];
        if (x_val > local_row_max) {
            local_row_sum *= __expf(local_row_max - x_val);
            local_row_max = x_val;
        }
...
torch::Tensor softmax3(const torch::Tensor& x) {
    ...
    softmax3_kernel<<<grid, block, smem_size>>>(x_flatten.data_ptr<float>(), y.data_ptr<float>(), rows, cols);
